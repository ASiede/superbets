...
describe('EventFormAnswer', () => {
  ...
  const wrapper: any = mount(
    <Provider store={store}>
      <EventFormAnswer answer={answer} questionId={questionId} />
    </Provider>
  );
  ...
  it('dispatches updateAnswer on answer input number change', () => {
    const mockValue = 'Who wins?';
    wrapper.find(InputNumber).at(0).props().onChange({ value: mockValue });
    expect(mockDispatch).toHaveBeenCalledWith(
      updateAnswer({
        questionId,
        answerId: answer.answerId,
